...
bool SemanticChecker::isChar(char* str) {
    if (str == nullptr || strlen(str) != 3) return false;
    return (str[0] == '\'' && str[2] == '\'');
}
...
bool SemanticChecker::isBool(char* str) {
    if (str == nullptr || strcmp(str, "") == 0) {
        return false; // null or empty string is considered false
    }
...
char* SemanticChecker::determineType(char* str) {
    if (isInt(str)) return (char*)"int";
    if (isFloat(str)) return (char*)"float";
    if (isString(str)) return (char*)"string";
    if (isChar(str)) return (char*)"char";
    if (isBool(str)) return (char*)"bool";
    return (char*)"unknown";
}
